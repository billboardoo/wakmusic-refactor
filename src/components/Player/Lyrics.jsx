...
function Lyrics(props) {
    const [subtitle] = useSubtitles({subtitles: `/static/lyrics/${props.id}.vtt`});
    const [loading, setLoading] = React.useState(false);
    const ran = Math.floor((Math.random() * 99) + 1);

    React.useEffect(() => {
        axios.get(`/api/check-lyrics/${props.id}`).then((r) => {
    ...
    if (loading || subtitle.length === 0) return <div className="lyrics-not-found">
        {ran <= 5 ? "무의식적으로 가사가 보이고 있습니다." : "이 노래는 가사가 제공되지 않습니다."}
    </div>;
    return (
        <div className="lyrics-wrap fadein">
            {subtitle.map(s =>
                <div aria-label={convertTime(s.stop)} onClick={() => moveToLyrics(convertTime(s.start))}
                     id={convertTime(s.start)} className="lyrics-text" key={s.stop}>{s.text}</div>
            )}
        </div>
    )
}
