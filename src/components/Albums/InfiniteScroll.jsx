...
const InfiniteScroll = (props) => {
    ...
    const fetchMoreData = async () => {
        setLoading(true);
        await axios.get(`/api/list/${props.type}?period=${props.time}&start=${start}`)
            .then((response) => {
    ...
    const getAlbums = () => {
        if (time === "201610") return <div id="ss" className="fadein">저희 왁타버스 뮤직을 이용해 주셔서 감사합니다.</div>
        ...
        item.forEach((album, i) => {
            data.push(
                <Link to={`/player/${album.id}`} className="albums-item fadein" key={i}>
                    ...
                    <img src={`https://i.ytimg.com/vi/${album.id}/hqdefault.jpg`} alt="" className="albums-image"/>
