...
const UpdateTime = (props) => {
    ...
    useEffect(() => {
        axios.get('/api/updated').then((item) => {
            setUpdate(item.data)
        })
    }, []);
    ...
    if (props.type === "daily") return <div className="updated fadein">일간 차트는 매일 0시에 업데이트됩니다.</div>
    if (props.type === "weekly") return <div className="updated fadein">주간 차트는 매주 월요일 0시에 업데이트됩니다.</div>
