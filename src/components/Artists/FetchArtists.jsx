...
const FetchArtists = (props) => {
    ...
    const fetchMoreData = async () => {
        setLoading(true);
        await axios.get(`/api/artist/albums/${artists[index].id}?start=${start}&sort=${type}`)
            .then((response) => {
    ...
    useEffect(() => {
        axios.get('/api/artist/list').then((i) => {
            setArtists(i.data)
        })
    }, []);
    ...
    const getAlbums = () => {
        ...
        items.forEach((i, count) => {
            albums.push(
                <div className={count % 2 === 0 ? "artist-albums bg-dark fadein" : "artist-albums fadein"} key={count}>
    ...
    const selectArtist = (e) => {
        ...
        for (let i = 0; i < artists.length; i++) {
            if (i === (typeof (e) === "number" ? e : parseInt(e.target.id))) {
                ...
                wrap.scrollTo({left: el.offsetLeft - wrap.clientWidth, behavior: 'smooth'});
                ...
                document.getElementsByClassName("select-bar-item")[i].className = "select-bar-item";
            }
        }
        return true;
    }
    ...
    const getArtists = () => {
        ...
        artists.forEach((i, count) => {
            items.push(
                <div key={count} id={String(count)} onClick={(e) => selectArtist(e)}
                     className={count === 0 ? "select-bar-item bar-selected fadein" : "select-bar-item fadein"}>
                    <div className="image-rounded-wrap" id={String(count)}>
                        <img src={`/static/artist/card/${i.id}.png`} alt="" className="image-rounded"
                             id={String(count)}/>
                    </div>
                    <div className="image-name" id={String(count)}>{i.short}</div>
                </div>
            )
            if (artists[count + 1] && artists[count + 1].group !== artists[count].group)
                items.push(
                    <div className="group-image-div fadein" key={(count + 1) * 100} id={artists[count + 1].group}>
                        <img src={`/static/artist/group/${artists[count + 1].group}.png`}
                             alt="" className="group-image" onClick={clickEvent}/>
                    </div>
                );
        })
        return items;
    ...
    useEffect(() => {
        ...
        if (!loading && start === 0 && artists[index]) fetchMoreData().then();
        ...
        <>
            <div id="select-section" className="fadein">
                <div className="select-bar-arrow">
                    <div className="arrow-active" id="arrow-right" onClick={() => props.scroll("right")}>
                    ...
                    <div className="arrow-inactive" id="arrow-left" onClick={() => props.scroll("left")}>
                        <svg width="10" height="18" viewBox="0 0 10 18" fill="none"
                             xmlns="http://www.w3.org/2000/svg">
                            <path d="M9.12403 1.45753L1.58156 9L9.12403 16.5425" strokeWidth="1.5"
                                  strokeLinecap="round" strokeLinejoin="round"/>
            ...
            <div id="artist-section" className="fadein">
                <ArtistDetail artists={artists} setType={setType} setIndex={setIndex} index={index}
                              selectArtist={selectArtist}/>
                <div id="albums-body">
                    <Dropdown setType={setType} setData={setItem} elements={SortTypeList}/>
