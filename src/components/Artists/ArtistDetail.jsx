    ...
    const getArtistDetail = () => {
        ...
        return (
            <div id="artist-body" className="fadein">
                ...
                <div className="artist-line" id={`artist-${item.id}`}>{item.title}</div>
                <div className="artist-image-wrap">
                    {item.id === "viichan" && <img src={oldImage} alt="" id="old-viichan"/>}
                    <img src={`/static/artist/full/${item.id}.png`} alt="" className="artist-image"/>
                </div>
                <div className="artist-arrow">
                    <div className="arrow-active" id="artist-prev" onClick={() => moveArtist('prev')}>
                        <svg width="9" height="14" viewBox="0 0 9 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M7.34315 1.34315L1.68629 7L7.34315 12.6569" strokeWidth="1.5" strokeLinecap="round"
                                  strokeLinejoin="round"/>
                        </svg>
                    </div>
                    <div className="arrow-active" id="artist-next" onClick={() => moveArtist('next')}>
                        <svg width="9" height="14" viewBox="0 0 9 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M1.65685 1.34315L7.31371 7L1.65685 12.6569" stroke="#8C95AF" strokeWidth="1.5"
                ...
                <div className="artist-account">
                    {item.youtube ? <a href={item.youtube} target="_blank" rel="noreferrer">
                        <img src={youtube} alt="" className="artist-sns"/></a> : null}
                    {item.twitch ? <a href={item.twitch} target="_blank" rel="noreferrer">
                        <img src={twitch} alt="" className="artist-sns"/></a> : null}
                    {item.instagram ? <a href={item.instagram} target="_blank" rel="noreferrer">
                        <img src={instagram} alt="" className="artist-sns"/></a> : null}
                </div>
                {item.id === "lilpa" &&
                    <div id="ee">
