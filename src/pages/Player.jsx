...
function Player() {
    ...
    useEffect(() => {
        if (location.state) {
            ...
            axios.get(`/api/search?type=ids&sort=new&keyword=${id}`).then((item) => {
                setData(item.data[0]);
            });
        }
    }, [id, location.state]);
    ...
    const time = (secs) => {
        ...
        return hour === 0 ? `${min}:${sec}` : `${hour}:${min < 10 ? '0' + min : min}:${sec}`;
    }
    ...
    const onPlayerReady = event => {
        ...
        document.getElementById('player-duration').innerHTML = time(Math.ceil(duration));
        document.getElementById('player-time').innerHTML = time(0);
        const progress = document.getElementById('player-progress')
        progress.value = 0;
        progress.style.background = '#ffffff66';
        document.getElementsByClassName('lyrics-wrap')[0]?.scrollTo({top: 0, behavior: 'smooth'});

        if (target.getPlayerState() % 2 === 0 || target.getPlayerState() === 3 || target.getPlayerState() === -1) {
    ...
    const onPlay = async event => {
        ...
        while (true) {
            ...
            document.getElementById('player-time').innerHTML = time(Math.ceil(current));

            progress.value = (current / duration) * 100;
            progress.style.background = 'linear-gradient(to right, #202F61 0%, #202F61 '
                + progress.value + '%, #ffffff66 '
                + progress.value + '%, #ffffff66 100%)';

            const lyrics = document.getElementsByClassName('lyrics-wrap')[0];
            if (lyrics && (lyricsEnd - 0.7 <= current || lyricsStart - 0.7 >= current)) {
                // eslint-disable-next-line no-loop-func
                Array.from(lyrics.children).forEach((item) => {
                    item.className = 'lyrics-text';
                    if (parseFloat(item.id) - 0.7 <= current && parseFloat(item.ariaLabel) - 0.7 >= current) {
            ...
            if (time(Math.ceil(current)) === time(duration) || target.getPlayerState() === 0) {
                moveNext();
            }
    ...
    return (
        <div className="container fadein" id="dark-container">
            <div id="player-area">
                <YouTube videoId={id} id="player" opts={{
                    playerVars: {
                        autoplay: 1,
                        rel: 0
                    }
                }} onReady={(e) => onPlayerReady(e)} onPlay={(e) => onPlay(e)}/>
                <div id="player-section">
                    <PlayerInfo data={data} duration={duration} movePrev={movePrev} moveNext={moveNext}/>
                </div>
