...
class Charts extends React.Component {
    ...
    componentDidMount() {
        ...
        axios.get(`/api/charts/${this.state.order}?limit=100`).then((item) => {
    ...
    render() {
        return (
            <div className="container fadein" id="dark-container">
                ...
                <div id="select-type">
                    <div id="list" className="type-selected" onClick={() => this.changeType("list")}>차트</div>
                    <div id="graph" onClick={() => this.changeType("graph")}>그래프</div>
                </div>
                {this.state.type === "list" ?
                    <FetchList data={this.state.data} setType={this.setType} setData={this.setData}
                               type={this.state.order}/> :
                    <FetchGraph data={this.state.data} setType={this.setType} setData={this.setData}
                                type={this.state.order}/>}
                <Ellipse/>
                <Footer/>
            </div>
