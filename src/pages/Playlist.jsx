...
const Playlist = () => {
    ...
    const getPlaylistdetail = () => {
        axios.get(`/api/playlist/detail/${playlistKey}`).then((res) => {
            ...
            if (res.data.clientId !== localStorage.getItem("clientId") || !localStorage.getItem("clientId")) {
                if (!res.data.public) {
                    navigate("/");
                }
    ...
    const changePlaylistTitle = () => {
        axios.post(`/api/playlist/edit/${playlistInfo.key}`, playlistInfo)
            .then((res) => {
    ...
    useEffect(() => {
        ...
        return () => {
            setMetaTags({
                ...
                description: "왁타버스 뮤직 핫 100, 노래 목록 등 왁엔터테인먼트 소속 가수들의 각종 음반 정보와 뉴스 제공"
            });
        }
    }, [playlistInfo]);
        ...
        <S.Container>
            ...
            {modifyModalBool && playlistInfo.clientId === localStorage.getItem("clientId") && (
                <Modal leftText="취소" leftEvent={() => setModifyModalBool(false)}
                    ...
                    <S.IntroduceText>변경할 재생목록의 이름을 입력해 주세요.</S.IntroduceText>
                    <S.NameInput onChange={playlistTitleChange} value={playlistInfo.title}
                                 placeholder="이름을 입력해 주세요."/>
                    {text !== "" && <div id="text-limit">{text}</div>}
                </Modal>
            )}
            {copyModalBool && (
                <Modal leftText="닫기" leftEvent={() => {
                    setCopyModalBool(false);
                    localStorage.removeItem("musicId");
                }} rightText="복사" rightEvent={onCopyLink}>
                    <S.ModalTitle>재생목록 공유</S.ModalTitle>
                    <S.ModalText>나만의 재생목록을 공유해 보세요!</S.ModalText>
                    <S.PlaylistLink ref={linkInputRef} onClick={onCopyLink}>{listLink}</S.PlaylistLink>
                </Modal>
            )}
            {deleteModalBool && (
                <Modal leftText="취소" leftEvent={() => setDeleteModalBool(false)}
                       rightText="삭제" rightEvent={deleteMusic}>
